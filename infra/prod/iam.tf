...
resource "aws_iam_role" "delete_recipe" {
  name = "delete_recipe_role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
...
resource "aws_iam_role_policy" "delete_recipe" {
  name = "delete_recipe_policy"
  role = aws_iam_role.delete_recipe.id
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
...
resource "aws_iam_role_policy_attachment" "delete_recipe" {
  role       = aws_iam_role.delete_recipe.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}
